...
def parse_line(line: str, line_number: int, regexp: Pattern = FULLY_PINNED_REGEX) -> ParsedLine:
    ...
    requirement_line_dict = {'raw': line, 'line_number': line_number, 'pinned': None}
    if match:
...
def parse_requirement_file(req_file: Path, regexp: Pattern = FULLY_PINNED_REGEX) -> List[Dict[str, Any]]:
    with open(req_file, 'r') as r_file:
        lines = r_file.readlines()
    parsed_requirements = list()
    for i, line in enumerate(lines, 1):
        parsed_requirements.append(requirement_line)
